...
def test(model, writer, device):
    ...
    for idx, (features, coors, shape, l_matrix) in enumerate(test_loader):
        ...
        data[idx, 1] = evaluate("jacobi", l_matrix, rhs, diags(1. / l_matrix.diagonal()))
        ...
        preconditioner = smoothed_aggregation_solver(l_matrix).aspreconditioner(cycle="V")
        preconditioner = csr_matrix(preconditioner.matmat(np.eye(shape[0], dtype=np.float32)))
